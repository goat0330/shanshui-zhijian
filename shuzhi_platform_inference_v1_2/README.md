...
这是比赛平台执行的单图六分类离线推理包。正式包只包含推理脚本、运行时元数据和四个 `reviewed_v2` 448px 四折 `last_stage` 最优权重，不包含训练集、测试集、标签文件、OOF、复核材料或旧提交文件。
...
输入先做 EXIF 转正和 RGB 转换；模型输入使用 448 尺寸等比缩放、均值色填充、BICUBIC 和 ImageNet mean/std。输出宽高来自文件原始尺寸。
