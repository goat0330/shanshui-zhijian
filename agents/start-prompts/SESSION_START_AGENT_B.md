# Session 启动提示词 — 工程可靠性_AGENT_B

> **架构说明**: 每个 Agent 使用独立的 GeoCode 项目上下文（独立窗口/CLI）。
> 本窗口只服务 RELIABILITY_AGENT_B。A/C/D/E 有各自独立的窗口和 worktree。
> 不要在本窗口打开或修改其他 Agent 的目录。

将以下内容完整粘贴到 GeoCode 中 agent-b worktree 的 Session：
...
【隔离声明】本窗口只服务 RELIABILITY_AGENT_B。其他 Agent 有各自独立的
...
5. agents/inbox/RELIABILITY_AGENT_B.md
...
`D:\研究生作业\人工智能实践比赛\山水智鉴_Git工作区\agent-b`，
或分支不是 `feature/agent-b-top-level-session`，立即停止。
