...
def link_objects_across_time(
    ...
    observation_id_field: str = "observation_id",
    ...
    Rules:
      ...
      - A-G0-1: observation_id from feature properties is propagated through Union-Find
        so each CandidateObject receives EXACT references to its members.

    Returns:
      List of CandidateObject with stable candidate_id
      and non-empty source_observation_ids.
    """
    ...
    for si, feats in enumerate(per_scene_features_crs):
        for fi, feat in enumerate(feats):
            ...
            records.append({
                ...
                # A-G0-1: observation_id from feature properties
                "observation_id": props.get(observation_id_field, ""),
            })
    ...
    for group_root, member_indices in groups.items():
        ...
        # A-G0-1: Collect exact observation_ids from member records
        obs_ids: list[str] = []
        seen_obs = set()
        for m in members:
        ...
        # A-G0-5: Real union_geometry using shapely unary_union
        union_geom = None
        try:
            from shapely.geometry import shape as sh_shape
            from shapely.ops import unary_union
            sh_geoms = [sh_shape(m["geometry"]) for m in members if m.get("geometry")]
            sh_geoms = [g if g.is_valid else g.buffer(0) for g in sh_geoms]
            if sh_geoms:
                u = unary_union(sh_geoms)
                if u.is_empty:
                    union_geom = rep_geom
                else:
                    union_geom = u.__geo_interface__
        except Exception:
            union_geom = rep_geom

        # A-G0-4: Stable candidate_id — no scene_index dependency
        # Use sorted(observation_refs) + change_type + candidate_rule_version + canonical union hash
        sorted_obs = sorted(obs_ids) if obs_ids else ["no-obs"]
        id_input = (
            f"{'|'.join(sorted_obs)}"
            f"|{ct_repr}"
            f"|candidate.v0.2"
        )
        # Add canonical geometry hash for stability across union precision changes
        if union_geom:
            try:
                # Canonical geometry: normalize ring start/direction via shapely
                sh_u = sh_shape(union_geom)
                normalized = sh_u.normalize() if hasattr(sh_u, 'normalize') else sh_u
                canonical_wkt = normalized.wkt if hasattr(normalized, 'wkt') else str(normalized)
                id_input += f"|{hashlib.sha256(canonical_wkt.encode()).hexdigest()[:16]}"
            except Exception as e:
                raise ValueError(
                    f"Cannot compute canonical geometry for candidate ID: {e}"
                ) from e
        cid = f"cand-{hashlib.sha256(id_input.encode()).hexdigest()[:20]}"
        ...
        candidates.append(CandidateObject(
            ...
            # A-G0-1: populated from feature observation_ids
...
def rank_candidates(
    ...
    A3 重构:
      ...
      - quality_factor 使用纯正交质量: 只基于水出现率稳定性，不 reuse robust_z
      - score_type: within_run_ranking (非概率)
      - 跨运行不可比: 使用运行内 max 归一化

    score = w_persistence * persistence_ratio
          + w_robust_z * normalized_robust_z   ← robust_z 只在此出现
          + w_area * normalized_area
          + w_quality * quality_factor          ← 纯正交: 水出现率稳定性，不含 robust_z
    ...
    for c in candidates:
        ...
        # A3: quality_factor 使用纯正交质量 — 不包含 robust_z (已有独立权重)
        # 仅基于水出现率稳定性: 适中范围(0.2-0.8)的检测更可靠，排除稳定水体/陆地
        quality_factor = 0.5  # 默认中等
        if c.water_occurrence_mean > 0:
            # 水出现率适中 (0.2-0.8) 的检测更可靠
            wq = 1.0 - abs(c.water_occurrence_mean - 0.5) * 2.0
            quality_factor = max(0.0, min(1.0, wq))
        ...
        c.score_components = {
            ...
            "quality_factor": {
                ...
                "description": "正交质量: 仅 water_occurrence_stability (不含 robust_z)",
            },
...
# ── Candidate to GeoJSON + JSON (通过 DetectionCandidate 模型) ──
...
def _candidate_object_to_detection_candidate(
    ...
    dc = DetectionCandidate(
        ...
        temporal_extent=TemporalExtent(start=c.first_seen, end=c.last_seen),
...
def candidates_to_geojson(candidates: list[CandidateObject]) -> list[dict]:
    ...
    for c in candidates:
        ...
        if not geom:
            continue
        dc_dict = _candidate_object_to_detection_candidate(c)
        props = dc_dict  # Use DetectionCandidate fields as properties
        features.append({
            "type": "Feature",
            "geometry": geom,
            "properties": props,
...
def candidates_to_json(candidates: list[CandidateObject],
    ...
    """Convert candidate objects to detailed JSON using DetectionCandidate schema."""
    return {
        "schema_version": "candidate.v0.3",
        ...
        "candidates": [
            _candidate_object_to_detection_candidate(c)
            for c in candidates
        ],
    }