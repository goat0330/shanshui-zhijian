...
确认：
- `git branch --show-current` 输出 `feature/agent-a-top-level-session`
- `git rev-parse --show-toplevel` 输出路径以 `agent-a` 结尾

如果不一致，**立即停止，报告项目经理_AGENT_E**。

### 工作纪律

- 所有代码变更只提交到 `feature/agent-a-top-level-session`
- 只修改 `agents/感知算法_AGENT_A.md` 中分配的允许目录
- 禁止修改 `agents/` 下其他 Agent 的文件
- 禁止 git switch 到其他分支
- 禁止提交到 integration、develop、main
- 禁止读取或修改 `agent-b`、`agent-c`、`agent-d`、`agent-e` 目录
...
1. 提交到 `feature/agent-a-top-level-session`
2. 按 `agents/HANDOFF_TEMPLATE.md` 写交接文档，放到 `agents/handoff/PERCEPTION_AGENT_A_to_E.md`
3. 通知项目经理_AGENT_E 审查
4. **不自行合并**
