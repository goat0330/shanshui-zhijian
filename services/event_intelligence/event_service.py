...
class EventService:
    ...
    def create_event(
        ...
        event_id = self._compute_event_id(candidate.candidate_id)

        # 检查是否已有 Event
        existing_event = self.repo.get_event(event_id)
        if existing_event:
            raise DuplicateEventError(
                f"Candidate {candidate.candidate_id} 的 Event 已存在: {event_id}"
            )
        ...
        # 构建 missing_context
        missing = []
        if bundle.modalities_missing:
            missing.append(f"missing_modalities:{','.join(bundle.modalities_missing)}")
        if candidate.quality_summary:
            vpr = candidate.quality_summary.get("valid_pixel_ratio", 1.0)
        ...
        self.repo.save_event_candidate_link(event.event_id, candidate.candidate_id)
    ...
    def apply_review_decision(self, review: ReviewDecision) -> tuple[AnomalyEvent, bool]:
        ...
        if review.action == "reclassify" and review.reason_code:
            event.category = review.reason_code

        # 确保 resulting_event_version 正确
        if review.resulting_event_version != event.event_version:
            # auto-fix
            object.__setattr__(review, "resulting_event_version", event.event_version)

        # 持久化（事务：同时写 Event 新版本 + Review + Replay）
        ...
        try:
            ...
            replay_entry = ReplayEntry(
                replay_entry_id=f"rpl_{review.decision_id}",
                event_ref=event.event_id,
                sequence_number=self.repo.get_latest_sequence_number() + 1,
                actor_type="human",
                actor_ref=review.reviewer_ref or "system",
            ...
            self.repo.append_replay_entry(replay_entry)
            self.repo.commit()
        except Exception:
            self.repo.rollback()
            raise
    ...
    @staticmethod
    def _build_reason_codes(candidate: DetectionCandidate) -> list[str]:
        codes = []
        if candidate.candidate_type == "water_extent_change":
