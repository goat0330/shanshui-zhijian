...
- git_commit, git_branch, git_dirty (three-state)
- Python 版本, OS, 关键依赖版本
- 输入 Asset ID, URI, SHA256, metadata_source, dataset_version
- 输出 Artifact ID, URI, SHA256, size_bytes, media_type
- 配置哈希, validation_policy
- 时间戳、状态、失败详情
- 运行命令和随机种子
- producer_version
- Manifest 自身 SHA256

提供 RunManifestRecorder (Protocol) 供 Agent A 在 SAR Tool 中调用。
"""
...
def git_dirty_three_state(cwd: str | None = None) -> str:
    """Three-state git dirty: 'true', 'false', or 'unknown'."""
    status = _git_cmd("status", "--porcelain", cwd=cwd)
    if status is None:
        return "unknown"
...
# ── RunManifestRecorder Protocol ──────────────────────────────────

class RunManifestRecorder(Protocol):
    """RunManifest 记录器协议，供 Agent A 在 SAR Tool 中调用。

    所有 Agent 通过此协议记录运行血缘，不依赖具体实现。
    """

    def record_start(self, run_id: str, task_id: str = "",
                     task_spec_ref: str = "") -> None:
        """记录运行开始。"""
        ...

    def record_input_asset(self, asset_id: str, uri: str = "",
                           sha256: str = "",
                           metadata_source: str = "unknown") -> None:
        """记录输入资产信息。"""
        ...

    def record_output_artifact(self, artifact_id: str, uri: str = "",
                               sha256: str = "", size_bytes: int = 0,
                               media_type: str = "") -> None:
        """记录输出产物信息。"""
        ...

    def record_failure(self, stage: str, error_type: str,
                       message: str) -> None:
        """记录失败信息。"""
        ...
    ...
    def finalize(self, status: RunStatus = RunStatus.SUCCEEDED
                 ) -> "RunManifest":
        """最终化并返回 Manifest。"""
        ...
...
class RunManifest(BaseModel):
    ...
    # ── 核心标识 ────────────────────────────────────────────────
    schema_version: str = "run-manifest.v0.2"
    ...
    git_dirty: str = Field(default_factory=lambda: git_dirty_three_state())
    git_root: str | None = Field(default=None)
    ...
    producer_version: str = Field(
        "", description="生产者版本号，如 sar_temporal_change_tool@v1.2.3")
    ...
    def compute_manifest_hash(self) -> str:
        ...
        排除动态字段（started_at, finished_at）和自身哈希。
        """
        data = self.model_dump(
            mode="json",
            exclude={"manifest_sha256", "started_at", "finished_at"},
        )
    ...
    def finalize(self, status: RunStatus = RunStatus.SUCCEEDED
        ...
        """完成 Manifest：设置状态和时间 → 脱敏路径 → 计算 SHA256。"""
        self.status = status
    ...
    def verify_hash(self) -> tuple[bool, str]:
        """验证 manifest_sha256 一致性。

        Returns:
            (True, computed_hash) if hash matches,
            (False, computed_hash) if mismatch.
        """
        if not self.manifest_sha256:
            return False, ""
        computed = self.compute_manifest_hash()
        return computed == self.manifest_sha256, computed


# ── RunManifestBuilder（实现 RunManifestRecorder）─────────────────

class RunManifestBuilder:
    """便捷构建 Run Manifest，实现 RunManifestRecorder 协议。"""
    ...
    def record_input_asset(self, asset_id: str, uri: str = "",
        ...
        if self._manifest is None:
            raise RuntimeError("Must call record_start first")
        self._manifest.input_assets.append(InputAssetRecord(
    ...
    def record_output_artifact(self, artifact_id: str, uri: str = "",
        ...
        if self._manifest is None:
            raise RuntimeError("Must call record_start first")
        self._manifest.output_artifacts.append(OutputArtifactRecord(
    ...
    def record_failure(self, stage: str, error_type: str,
        ...
        if self._manifest is None:
            raise RuntimeError("Must call record_start first")
        self._manifest.failure = FailureInfo(
            stage=stage, error_type=error_type, message=message)
        self._manifest.status = RunStatus.FAILED
    ...
    def finalize(self, status: RunStatus = RunStatus.SUCCEEDED
        ...
        if self._manifest is None:
            raise RuntimeError("Must call record_start first")
        return self._manifest.finalize(status)
    ...
    @staticmethod
    def fail(manifest: RunManifest, stage: str, error_type: str,
             message: str) -> RunManifest:
        manifest.failure = FailureInfo(
            stage=stage, error_type=error_type, message=message)
