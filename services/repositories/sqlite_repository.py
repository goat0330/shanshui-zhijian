...
class SQLiteRepository(Repository):
    ...
    # ── AnomalyEvent ────────────────────────────────────────────
    def save_event(self, event: AnomalyEvent, commit: bool = True) -> AnomalyEvent:
        conn = self._get_conn()
        conn.execute("""
    ...
    # ── Event Version ──
    def save_event_version(self, event: AnomalyEvent, commit=True) -> AnomalyEvent:
        import json as _json
        snap = _json.dumps(event.model_dump(mode='json'), ensure_ascii=False, default=str)
        conn = self._get_conn()
        conn.execute("INSERT OR REPLACE INTO event_versions(event_id,event_version,snapshot,created_at) VALUES(?,?,?,?)",
                     (event.event_id, event.event_version, snap, datetime.now().isoformat()))
        if commit: conn.commit()
        return event

    def get_event_version(self, event_id: str, version: int) -> AnomalyEvent | None:
        conn = self._get_conn()
        row = conn.execute("SELECT snapshot FROM event_versions WHERE event_id=? AND event_version=?", (event_id, version)).fetchone()
        if not row: return None
        import json as _json
        return AnomalyEvent(**_json.loads(row["snapshot"]))

    def list_event_versions(self, event_id: str) -> Sequence[AnomalyEvent]:
        rows = self._get_conn().execute("SELECT snapshot FROM event_versions WHERE event_id=? ORDER BY event_version", (event_id,)).fetchall()
        import json as _json
        return [AnomalyEvent(**_json.loads(r["snapshot"])) for r in rows]

    # ── Optimistic Lock ──
    def optimistic_update_event(self, event: AnomalyEvent, expected_version: int, commit=True) -> int:
        import json as _json
        cur = self._get_conn().execute(
            "UPDATE events SET event_version=?,status=?,category=?,severity=?,geometry=?,temporal_extent=?,candidate_refs=?,evidence_bundle_refs=?,reason_codes=?,missing_context=?,previous_version_ref=?,updated_at=? WHERE event_id=? AND event_version=?",
            (event.event_version, event.status, event.category, event.severity,
             _json.dumps(event.geometry) if event.geometry else None,
             _json.dumps(event.temporal_extent) if event.temporal_extent else None,
             _json.dumps(event.candidate_refs) if event.candidate_refs else '[]',
             _json.dumps(event.evidence_bundle_refs) if event.evidence_bundle_refs else '[]',
             _json.dumps(event.reason_codes) if event.reason_codes else '[]',
             _json.dumps(event.missing_context) if event.missing_context else '[]',
             event.previous_version_ref, event.updated_at, event.event_id, expected_version))
        if commit: self._get_conn().commit()
        return cur.rowcount

    # ── Asset Registry ──
    def asset_registry_exists(self, asset_id: str) -> bool:
        r = self._get_conn().execute("SELECT 1 FROM evidence WHERE source_asset_ref=? LIMIT 1", (asset_id,)).fetchone()
        return r is not None

    def seed_asset_registry(self, asset_ids: list[str]) -> None:
        now = datetime.now().isoformat()
        for aid in asset_ids:
            try:
                self._get_conn().execute("INSERT INTO evidence(evidence_id,evidence_type,source_modality,source_asset_ref,created_at) VALUES(?,?,?,?,?)",
                                         (f"__reg_{aid}", "registry", "SYSTEM", aid, now))
            except Exception:
                pass  # already exists
        self._get_conn().commit()

    # ── ReviewDecision ──────────────────────────────────────────
    def save_review(self, review: ReviewDecision, commit: bool = True) -> ReviewDecision:
        conn = self._get_conn()
        conn.execute("""
    ...
    def get_latest_sequence_number(self, event_ref: str | None = None) -> int:
        conn = self._get_conn()
        if event_ref is None:
            ...
            row = conn.execute(
                ...
                "FROM replay_entries WHERE event_ref = ?",
                (event_ref,),
            ).fetchone()
